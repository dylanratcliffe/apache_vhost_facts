Facter.add(:apache_vhosts) do
  ...
  @unnamed_vhost_line     = /^(?<ip>.*):(?<port>\d+)\s+(?<name>.*)\s\((?<location>.*)\)/
  @named_first_line       = /^(?<ip>.*):(?<port>\d+)\s+is\sa.*/
  @named_subsequent_lines = /^\s+(?<default>(default server)?)(port (?<port>\d+)\snamevhost)?\s(?<name>.*)\s\((?<location>.*:(?<line>\d+))\)/
